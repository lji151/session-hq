...
세션마다 열어서 직접 타이핑하는 대신 오케스트레이터에게 말한다. 요청은 `dispatches.md`에 기록되고,
해당 부서 세션이 다음에 열릴 때 그것을 집어 가서 결과를 보고하며, 오케스트레이터가 검토한다.
...
디스크에 남은 dispatch가 기록이고, 살아 있는 세션에 보내는 메시지는 조금 더 빨리 눈에 띄게 하는
수단일 뿐이다. 메시지를 전달할 수 있는지 자체는 하네스의 영역이며, HQ가 보장하는 것은 요청과 결과가
디스크에 남는다는 것뿐이다. 그 덕분에 사흘 전에 닫아 둔 부서 세션에도 이 방식이 통한다.

부서 세션은 자기 상태 파일보다 앞서, 주입문 맨 위에서 열린 dispatch를 본다. 마무리는
`hq.mjs done <id> --note "<결과>"`. 오케스트레이터의 대기열은 대시보드의 **Awaiting review** 목록이다.

### 각자 방식대로 쓰기

위의 어느 것도 정해진 워크플로가 아니다. 고정된 것은 상태 파일 형식뿐이고, 세션을 어떻게 나눌지와
HQ를 어떻게 소비할지는 전부 조절 가능한 값이다. 실제로 쓰이는 여섯 가지 형태:
...
수도 있다. `hq.config.json`의 `domains`를 고치고 해당 `status-<domain>.md` 파일명을 바꾸면 끝이며,
그것이 무엇을 뜻하는지는 나머지 어디에서도 신경 쓰지 않는다.

첫 번째부터 시작하라. 인수인계를 한 번 놓쳐서 아쉬워졌을 때 그때 다이얼을 늘리면 된다.
